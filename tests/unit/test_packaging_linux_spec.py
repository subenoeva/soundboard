...
def test_spec_collects_the_pedalboard_and_onnxruntime_runtimes() -> None:
    ...
    extension, and onnxruntime loads its provider libraries from onnxruntime/capi at
    runtime."""
    source = SPEC_PATH.read_text()
    tree = ast.parse(source)
    call = _analysis_call(source)
    ...
    assert "pedalboard_native" in _literals(_keyword_value(call, "hiddenimports")), (
        "pedalboard imports its extension as a separate top-level module"
    )


def test_spec_keeps_the_xlib_binaries_when_adding_the_new_collections() -> None:
    """binaries= used to be xlib_binaries outright; unpacking the new collections into
    a list is exactly the edit that can drop it."""
    call = _analysis_call(SPEC_PATH.read_text())

    assert "xlib_binaries" in _starred_names(_keyword_value(call, "binaries"))
...
def test_the_existing_bundle_contents_survive_the_new_entries() -> None:
    ...
    assert {"pynput.keyboard._xorg", "pynput.mouse._xorg"} <= hidden
    assert "xlib_hiddenimports" in _starred_names(_keyword_value(call, "hiddenimports"))
    assert 'collect_submodules("keyring.backends")' in source
    assert "rt_hook_portaudio.py" in source
